...
def build_app(root: Path):
        ...
        return projection.build_snapshot(
            ...
            health=(
                HealthDimension(
                    ...
                    reason="repository-local service projection",
                ),
            ),
    ...
    return create_command_center_app(
        snapshot_provider=snapshot,
        event_broker=RealtimeEventBroker(),
        inbox=AttentionNotificationBroker(),
        application_provider=lambda: RepositoryApplicationProjectionBuilder(root).build(snapshot()),
        auth=CommandCenterAuth(validate),
    )


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the bounded Project Pipeline Command Center service"
    )
    ...
    if args.check:
        print(
            json.dumps(
                {
                    "status": "SOURCE_RUNTIME_CHECK_PASS",
                    "route_count": len(app.routes),
                    "auth_token_configured": bool(
                        os.environ.get("PROJECT_PIPELINE_COMMAND_CENTER_TOKEN")
                    ),

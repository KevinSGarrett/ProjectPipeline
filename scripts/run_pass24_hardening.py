...
from project_pipeline.verification.performance import (
    cli_help_performance,
    repository_validation_performance,
)
...
def main() -> int:
    ...
    if args.measure_performance:
        perf = {
            "repository_validator": repository_validation_performance(root, samples=3).model_dump(
                mode="json"
            ),
            "cli_help": cli_help_performance(root, samples=5, p95_budget_ms=2500).model_dump(
                mode="json"
            ),
        ...
        if not all(
            item.get("passed", True) for key, item in perf.items() if isinstance(item, dict)
        ):
            return 2

...
The package CLI is exposed through `project-pipeline` / `python -m project_pipeline`. `python -m project_pipeline --help` is the executable command inventory. `python -m project_pipeline validate --root .` validates a standalone source checkout without requiring private delivery records; in an initialized ProjectPipeline workspace it also validates that workspace's project-control contract.

...
def test_ppdb_0018_store_and_rollback(project_root, tmp_path):
    ...
    with LifecycleStore(db, project_root) as s:
        ...
        ids = {x[0] for x in s.db.execute("SELECT migration_id FROM schema_migrations")}
        assert "PPDB-0025" not in ids and "PPDB-0024" in ids
        runner.rollback_last()
        ids = {x[0] for x in s.db.execute("SELECT migration_id FROM schema_migrations")}
        assert "PPDB-0024" not in ids and "PPDB-0023" in ids
        runner.rollback_last()
        ids = {x[0] for x in s.db.execute("SELECT migration_id FROM schema_migrations")}
        assert "PPDB-0023" not in ids and "PPDB-0022" in ids
        runner.rollback_last()
        ids = {x[0] for x in s.db.execute("SELECT migration_id FROM schema_migrations")}
        assert "PPDB-0022" not in ids and "PPDB-0021" in ids
        runner.rollback_last()
        ids = {x[0] for x in s.db.execute("SELECT migration_id FROM schema_migrations")}
        assert "PPDB-0021" not in ids and "PPDB-0020" in ids
        runner.rollback_last()
        ids = {x[0] for x in s.db.execute("SELECT migration_id FROM schema_migrations")}

...
def test_ppdb_0012_is_reversible_without_damaging_ppdb_0011(tmp_path: Path):
    ...
    after_retrieval = runner.rollback_last()
    assert after_retrieval.latest_applied == "PPDB-0024"
    after_observations = runner.rollback_last()
    assert after_observations.latest_applied == "PPDB-0023"
    after_owner = runner.rollback_last()
    assert after_owner.latest_applied == "PPDB-0022"
    after_campaign = runner.rollback_last()
    assert after_campaign.latest_applied == "PPDB-0021"

CREATE TABLE IF NOT EXISTS command_center_events (
  event_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, sequence BIGINT NOT NULL,
  event_type TEXT NOT NULL, occurred_at_utc TIMESTAMPTZ NOT NULL, payload_json JSONB NOT NULL
);
...
CREATE TABLE IF NOT EXISTS command_center_inbox (
  ...
  created_at_utc TIMESTAMPTZ NOT NULL, payload_json JSONB NOT NULL
);
CREATE INDEX IF NOT EXISTS idx_command_center_inbox_open ON command_center_inbox(project_id,state,priority_score DESC);
CREATE TABLE IF NOT EXISTS command_center_notifications (
  ...
  suppressed BOOLEAN NOT NULL, decided_at_utc TIMESTAMPTZ NOT NULL, payload_json JSONB NOT NULL
);
CREATE TABLE IF NOT EXISTS command_center_control_requests (
  ...
  status TEXT NOT NULL, created_at_utc TIMESTAMPTZ NOT NULL DEFAULT NOW(), payload_json JSONB NOT NULL
);

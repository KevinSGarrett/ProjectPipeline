terraform {
  ...
  # Backend is intentionally declarative-only here. Concrete account/bucket/table
  # values must be injected at init time via explicit operator-approved backend config.
  backend "s3" {}
}
...
locals {
  enabled = var.enable_cloud_spine ? 1 : 0
...
resource "aws_s3_bucket" "recovery" {
  count         = local.enabled
  bucket_prefix = "${local.prefix}-recovery-"
}

resource "aws_s3_bucket_public_access_block" "recovery" {
  count  = local.enabled
  bucket = aws_s3_bucket.recovery[0].id

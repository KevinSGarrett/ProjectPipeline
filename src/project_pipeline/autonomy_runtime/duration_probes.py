...
def _probe_github(root: Path, candidate: dict[str, Any], expected_sha: str) -> dict[str, Any]:
    ...
    main = _run_json(["gh", "api", f"repos/{slug}/commits/main"], cwd=root)
    pulls = _run_json(
        ["gh", "pr", "list", "--repo", slug, "--state", "open", "--json", "number"], cwd=root
    )
    branches = _run_json(["gh", "api", f"repos/{slug}/branches?per_page=100"], cwd=root)
    raw_main_payload = main.get("payload")
    main_payload: dict[str, Any] = raw_main_payload if isinstance(raw_main_payload, dict) else {}
    actual_sha = str(main_payload.get("sha") or "")
    ...
    release_observation: dict[str, Any] = {"available": False}
    if isinstance(release, dict) and release.get("release_id") is not None:
        observed = _run_json(
            ["gh", "api", f"repos/{slug}/releases/{int(release['release_id'])}"], cwd=root
        )
        raw_release_payload = observed.get("payload")
        payload: dict[str, Any] = (
            raw_release_payload if isinstance(raw_release_payload, dict) else {}
        )
        expected_assets = {
            str(item.get("name"))
        ...
        release_observation = {
            "available": observed.get("exit_code") == 0,
            "draft": payload.get("draft") is True,
            "target_commitish": payload.get("target_commitish"),
            "asset_names_match": actual_assets == expected_assets and bool(actual_assets),
        }
    reads_ok = all(item.get("exit_code") == 0 for item in (main, pulls, branches))
    return {
        "ok": reads_ok
        and actual_sha == expected_sha
        and bool(candidate.get("identity_ok"))
        and release_observation.get("available") is True
        and release_observation.get("draft") is True
        and release_observation.get("target_commitish") == expected_sha
        and release_observation.get("asset_names_match") is True,
        "main_sha": actual_sha,
        "open_pull_count": len(pulls.get("payload") or [])
        if isinstance(pulls.get("payload"), list)
        else None,
        "branch_count": len(branches.get("payload") or [])
        if isinstance(branches.get("payload"), list)
        else None,

...
def verify_command_center_ui(
    ...
    *,
    ...
    output_dir = root / "evidence/verification/command_center_ui"
        ...
        try:
            for width, height in viewports:
                ...
                checks["all_surfaces"] = all(
                    page.locator(f"#{section}").count() == 1 for section in required_sections
                )
                ...
                initial_screenshot = output_dir / f"command_center_{width}x{height}_initial.png"
                ...
                checks["incident_resolve_blocked_before_verify"] = (
                    "resolve remains blocked"
                    in page.locator("#incident-status").inner_text().lower()
                )
                ...
                viewport_screenshot = output_dir / f"command_center_{width}x{height}_viewport.png"
                page.screenshot(path=str(viewport_screenshot), full_page=False)
                full_screenshot = output_dir / f"command_center_{width}x{height}_full.png"
                ...
                viewport_results.append(
                    {
                        ...
                        "passed": all(checks.values()) and not console_errors,
                    }
                )
                page.close()
            ...
            forced_colors_screenshot = output_dir / "command_center_forced_colors_1440x900.png"
    ...
    target = root / "evidence/command_center_ui_browser_accessibility.json"
    target.write_text(

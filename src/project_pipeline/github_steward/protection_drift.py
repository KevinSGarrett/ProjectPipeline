...
def evaluate_protection_drift(
    ...
    *,
    ...
    expected = expected_main_protection(
        observed.repository_slug, branch=observed.branch, required_checks=required_checks
    )
    drifts: list[str] = []
    ...
    if tuple(sorted(observed.required_status_checks)) != tuple(sorted(required_checks)):

        ...
        <div className="content-grid">
          <div className="main-column">
            ...
            <Section id="fleet" kicker="FLEET" title="Three-host fleet">
              <div className="table-wrap"><table><caption className="sr-only">Declared versus observed fleet hosts</caption><thead><tr><th>Host</th><th>Role</th><th>State</th><th>Freshness</th><th>CPU / RAM / Disk</th><th>GPU</th><th>Eligibility</th><th>Jobs</th><th>Lease</th><th>Assignment</th><th>Actions</th></tr></thead><tbody>{(fleet.hosts || []).length ? (fleet.hosts || []).map((host) => <tr key={host.machine_id}><td><strong>{host.hostname}</strong><small>{host.machine_id}</small></td><td>{host.role}</td><td><Badge tone={host.state === "READY" && host.freshness === "fresh" ? "good" : "warn"}>{host.state}</Badge></td><td><Badge tone={host.freshness === "fresh" ? "good" : "warn"}>{host.freshness}</Badge></td><td>{host.cpu_slots} / {host.memory_mb} / {host.disk_mb}</td><td>{host.gpu_name || "none"}{host.cuda_compute_capability != null ? ` CC${host.cuda_compute_capability}` : ""}</td><td>{host.eligibility}{(host.denial_reasons || []).length ? <small>{host.denial_reasons.join(", ")}</small> : null}</td><td>{host.active_jobs == null ? "unknown" : String(host.active_jobs)}</td><td>{host.lease_id == null || host.lease_id === "" ? "unknown" : host.lease_id}</td><td>{host.assignment == null || host.assignment === "" ? "unknown" : host.assignment}</td><td><div className="inbox-actions"><button type="button" aria-label={`Drain ${host.machine_id}`} onClick={() => fleetAction(host.machine_id, "drain")}>Drain</button><button type="button" aria-label={`Resume ${host.machine_id}`} onClick={() => fleetAction(host.machine_id, "resume")}>Resume</button></div></td></tr>) : <tr><td colSpan={11}>No live fleet observation; hosts are unknown until the authenticated fleet API returns.</td></tr>}</tbody></table></div>
